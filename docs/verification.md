...
| OpenCode | 七个 Skill 链接均指向当前源码；运行时发现需手动核验 |
| Claude Code | `claude plugin list --json` 提供版本、enabled 与安装路径；检查器从该路径核对七个 Skill 和两个 Hook |
| WorkBuddy | 插件页显示当前仓库版本、enabled、七个 Skill 和两个 Hook |
| ZCode | Settings → Plugins 显示当前仓库版本、7 Skills、2 Hooks；Skills 中存在 `scd-quickdev` |
...
读取五个平台的安装形态、Skill 根、Hook 和验证入口，并使用以下状态：
...
仓库无效。Codex 与 OpenCode 检查分别遵循 `CODEX_HOME` 和
`XDG_CONFIG_HOME`。检查器不会安装、更新、覆盖、重启或重新加载任何 Agent。
