...
9. When the user explicitly asks to review a completed delivery, use the verified Issue boundary, pull request, confirmed review findings, and acceptance evidence as candidate sources. A merged PR, passing suite, or `REVIEW_PASS` alone does not prove a reusable lesson. This review never starts automatically and never authorizes a write.
