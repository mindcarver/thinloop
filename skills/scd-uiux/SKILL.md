...
UX 契约还必须单独决定是否需要“实现后最终视觉确认”。普通页面和明确局部变更默认“不需要”；整体改版、高保真品牌表达，或布局、密度、视觉语言本身属于主观关键验收时标记“需要”，并说明触发原因。该确认在 QuickDev 生成真实页面对照证据后执行，不阻止设计契约先进入 `ready`，但在确认完成前阻止最终交付和关闭 Issue。
...
- 复杂交互存在可练习的非生产原型；
- 需要用户确认的重大视觉交付已经明确确认；
- 实现后最终视觉确认已明确标记为“需要”或“不需要”；
- 视觉产物与 UX 契约一致；
- 工程接口需求已经统一，或已明确分配给 `scd-architecture` 的共享契约步骤；
- 没有待定事项会实质改变可见行为或验收。

与 Discovery 组合时，把就绪的 UX 结论、视觉交付和产品契约变更送回一次合并确认。交给生产实施时，向 `scd-quickdev` 提供：已确认的产品契约、已就绪的 UX 契约、视觉 ID 与项目内视觉文件、适用的非生产原型、实现后最终视觉确认标记、已启用且就绪的架构或功能设计，以及共享接口契约。本技能不得实施生产前端代码。
