...
- 父 Execute 作为分支和工作树创建者，也是通道资源的清理所有者；
- 明确文件或模块所有权；
- 当前 Initiative 和图版本；
- 适用的产品与技术契约；
- 一次 `scd-quickdev` 调用；
...
父 Execute 会话保持可观察并负责协调。它记录每条通道的精确任务工作树、本地任务分支和远端任务分支，并在合并后按 QuickDev 契约提供终态清理证据。无法监控和协调的脱离后台工作不是执行通道。
...
2. 合并一个拉取请求；
3. 同步默认分支；
4. 删除已合并通道的精确工作树、本地任务分支和远端任务分支并验证不存在；
5. 安全变基或更新剩余通道；
6. 重新运行受新基准影响的证据；
7. 清理证据闭合后才按 QuickDev 契约关闭 Issue；
8. 选择更多工作前重建并验证依赖图。

假设发生变化的兄弟通道必须暂停并重新验证。合并后的基准实质改变其行为或验证衔接点时，通道此前的绿色结果已陈旧。

通道工作树有未提交状态时不得强制删除。把该通道标为 `BLOCKED`，记录精确状态并保持 Delivery Issue 开放；不得让 QuickDev 子通道和父 Execute 互相等待对方清理。
